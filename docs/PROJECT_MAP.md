...
- `src/pages/Assets.tsx` is the simple asset ledger for savings books and gold purchase lots. Active family members can create/edit/close/archive savings books, record interest/withdrawal/fee/settlement movements, update the shared gold buy-back price, and record partial or full gold sales. Archived rows remain read-only.
...
- Migrations `202609160001_asset_management.sql` and `202609160002_fix_asset_transaction_writes.sql` add the asset schema and repair linked transaction writes. Follow-up migration `202609160003_member_asset_controls.sql` stores one family-level gold buy-back price, calculates savings maturity in PostgreSQL, and expands the audited asset RPCs to active family members. Asset mutations atomically create linked `transactions` with source `asset`: deposits and gold purchases are expenses, interest/withdrawals/gold sales are income, and savings fees are expenses. Gold sales do not add a fee. Branch follow-up migration `202609160004_backfill_gold_expense_type.sql` ensures existing families have an active `Đầu tư vàng` category.
- `get_asset_summary` returns all-time actual net cash (`income - expenses`), the shared gold buy-back price and the current savings/gold list for the Dashboard, independent of the selected spending-period charts.
...
- `gold_assets` stores one purchase lot per buy date/price; `gold_sales` stores partial/full sales and their linked income transactions. The estimated sell value uses the manually entered family-level shop buy-back price in `families.gold_buyback_price_per_chi`.
