...
- Code trên `main` đã gồm PR [#231](https://github.com/nhan0805/family-expense/pull/231), làm mới panel cài đặt giá mua vào vàng dùng chung; PR [#232](https://github.com/nhan0805/family-expense/pull/232), loại bỏ khoảng trống ngang không cần thiết của bảng giao dịch; cùng các release trước.
...
- `Members` hiện phân biệt `false` với trạng thái preflight lỗi/null: chỉ chặn khi có giao dịch đang hoạt động, còn lỗi preflight vẫn cho phép xác nhận để `delete_empty_family` tự quyết định. `AppContext` dọn các key local theo `family_id` và các catalog sau khi xóa thành công ở local hoặc cloud.
...
- Kiểm thử local: Vitest 49 file/240 test, TypeScript, ESLint, Vite build và `git diff --check` pass; Playwright local chạy 6 test auth-dependent và cả 6 được skip theo guard hiện có. CI hậu merge [run 35359520727](https://github.com/nhan0805/family-expense/actions/runs/35359520727) pass quality, E2E, db-security và performance budget.
- Triển khai: PR [#225](https://github.com/nhan0805/family-expense/pull/225) merge vào `main` tại commit `663e44e`; [Cloudflare Pages check](https://dash.cloudflare.com/?to=/07ec67956cee45221fb1e3c98510c65a/pages/view/family-expense/5b8cf9a5-f601-4bd0-b754-3255299d064c) pass; smoke production `/`, `/dang-nhap` và `/thanh-vien` trả HTTP 200. Không có migration Supabase mới.
...
### Hiển thị lỗi khi thao tác xóa gia đình không thành công

- Nguyên nhân: nếu lời gọi RPC bị reject ở tầng network/runtime thay vì trả object `{ error }`, promise thoát khỏi handler nên không hiển thị lỗi và trạng thái nút có thể bị kẹt.
- `AppContext` hiện bắt cả lỗi RPC trả về và lỗi reject; màn hình Thành viên dùng `try/catch/finally`, hiển thị `Đang xóa gia đình…` và luôn kết thúc loading để người dùng có thể thử lại. Thêm regression test cho lỗi timeout.
- Files: `src/context/AppContext.tsx`, `src/pages/Members.tsx`, `src/pages/Members.test.tsx`. Không đổi schema, RLS/RPC hoặc dữ liệu.
- Kiểm thử: Vitest 48 file/233 test, TypeScript, ESLint, Vite build, E2E, db-security và performance budget đều pass qua CI.
- Triển khai: PR [#220](https://github.com/nhan0805/family-expense/pull/220) merge tại commit `6cae993`; [Cloudflare Pages check](https://dash.cloudflare.com/?to=/07ec67956cee45221fb1e3c98510c65a/pages/view/family-expense/92188dea-0df1-4ed0-9a4c-0f5bcc8b15d8) pass; không có migration Supabase mới; smoke production `/`, `/dang-nhap` và `/thanh-vien` trả HTTP 200.
...
- Kiểm thử: Vitest 48 file/234 test, TypeScript, ESLint, Vite build, E2E, db-security và performance budget đều pass qua CI.
- Triển khai: PR [#222](https://github.com/nhan0805/family-expense/pull/222) merge tại commit `4ec799c`; [Cloudflare Pages check](https://dash.cloudflare.com/?to=/07ec67956cee45221fb1e3c98510c65a/pages/view/family-expense/df2c7ab6-3982-4ef9-81a0-3fe9c5927706) pass; không có migration Supabase mới; live bundle đã xác nhận các nhãn trạng thái và smoke production `/`, `/dang-nhap`, `/thanh-vien` trả HTTP 200.
...
- Card lô vàng chuyển sang bố cục ba vùng từ breakpoint `md`: thông tin lô, hai giá trị mua/bán ước tính và nhóm thao tác; ở điện thoại nhỏ các vùng vẫn xếp dọc để giữ nhịp đọc và vùng chạm rõ ràng.
- Files: `src/pages/Assets.tsx`, `src/pages/Assets.ui.test.tsx`. Không đổi schema, RLS/RPC, Edge Function hoặc dữ liệu.
- Kiểm thử: local Vitest 48 file/225 test, TypeScript, ESLint, Vite build và `git diff --check` pass; CI quality, E2E, db-security và performance budget pass.
- Deployment: PR [#209](https://github.com/nhan0805/family-expense/pull/209) merge tại commit `cc411e024993906bc4fb5ec7820ce5dccc4e07f6`; [Cloudflare Pages check](https://dash.cloudflare.com/?to=/07ec67956cee45221fb1e3c98510c65a/pages/view/family-expense/36949a0f-2214-4fe7-8aa5-774be5c85e82) pass; smoke `https://family-expense-8fo.pages.dev/` trả HTTP 200.
