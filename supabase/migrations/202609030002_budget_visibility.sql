-- Phase 7: allow a purpose to stay available for transactions while being
-- excluded from monthly budget tracking. Existing budgets are retained so the
-- setting is reversible.

alter table public.purposes
  add column if not exists budget_enabled boolean not null default true;

create index if not exists purposes_budget_tracking_idx
  on public.purposes(family_id, active, budget_enabled);
...
begin
  ...
  if not exists (
    ...
    raise exception 'PURPOSE_NOT_BUDGETABLE';
  end if;
...
begin
  ...
  insert into public.budgets(family_id, year, month, purpose_id, amount, warning_threshold)
  select b.family_id, p_target_year, p_target_month, b.purpose_id, b.amount, b.warning_threshold
  from public.budgets b
  join public.purposes p
    on p.id = b.purpose_id
    and p.family_id = b.family_id
  where b.family_id = p_family_id
    and b.year = p_source_year
    and b.month = p_source_month
    and p.active = true
    and p.budget_enabled = true
  on conflict (family_id, year, month, purpose_id)
  do update set
