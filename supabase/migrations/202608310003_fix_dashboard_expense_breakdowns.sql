-- Khôi phục breakdown Chi tiêu chỉ lấy dòng tiền ra sau khi sửa chart Thu ròng.
...
begin
  ...
  purpose_data as (select p.name,sum(case when t.transaction_type in ('Chi tiêu','Tạm ứng') then t.amount when t.transaction_type='Hoàn tiền' then -t.amount else 0 end) value from actual_month t join public.purposes p on p.id=t.purpose_id group by p.id,p.name having sum(case when t.transaction_type in ('Chi tiêu','Tạm ứng') then t.amount when t.transaction_type='Hoàn tiền' then -t.amount else 0 end)>0 order by value desc),
  expense_data as (select e.name,sum(case when t.transaction_type in ('Chi tiêu','Tạm ứng') then t.amount when t.transaction_type='Hoàn tiền' then -t.amount else 0 end) value from actual_month t join public.expense_types e on e.id=t.expense_type_id group by e.id,e.name having sum(case when t.transaction_type in ('Chi tiêu','Tạm ứng') then t.amount when t.transaction_type='Hoàn tiền' then -t.amount else 0 end)>0 order by value desc),
