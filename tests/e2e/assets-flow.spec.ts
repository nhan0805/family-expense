...
test('luồng demo thêm và bán một dòng vàng', async ({ page }) => {
  ...
  await page.getByLabel('Giá tiệm mua vào ước tính / chỉ').fill('8500000');
  await page.getByRole('button', { name: 'Lưu dòng vàng' }).click();
  await page.getByRole('alertdialog').getByRole('button', { name: 'Tạo và ghi giao dịch' }).click();
  await expect(page.getByText('Đã lưu dòng vàng.')).toBeVisible();
