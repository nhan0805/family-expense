...
5. Update `CHANGELOG.md` with what changed, files/DB objects, validation and expected deployment state.
...
8. Rewrite `HANDOFF.md` as a current snapshot. Remove stale pending/deployment statements; do not append a full session history.
9. For a deploy request, stage and commit code plus the latest `HANDOFF.md`/`CHANGELOG.md`, push the feature branch, create/update the PR to `main`, keep it current, enable auto-merge and monitor required checks.
