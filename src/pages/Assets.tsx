...
function GoldRow({
  ...
  const isClosed = sold || asset.status === 'archived';
  const statusLabel = isClosed ? (en ? 'Sold' : 'Đã bán') : (en ? 'Active' : 'Đang giữ');
  const statusClass = isClosed
    ? 'border-gray-300 bg-gray-100 text-gray-600 dark:border-white/15 dark:bg-white/[.06] dark:text-gray-300'
  ...
  return (
    <article className="asset-gold-row rounded-2xl border border-black/10 bg-black/[.015] p-3 shadow-sm transition-colors hover:border-[var(--primary)] sm:p-4 dark:border-white/10 dark:bg-white/[.025]">
      <div className="asset-row-main grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.75fr)_auto] lg:items-center">
