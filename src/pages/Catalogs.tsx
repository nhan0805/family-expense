...
function Catalog({
  ...
  return <section className="catalog-card card p-4 sm:p-5">
    <div className="catalog-card-header mb-3"><h3 className="font-bold lg:whitespace-nowrap">{title}</h3>{canManage && <button className="catalog-add-button btn-secondary flex items-center gap-1 text-sm" onClick={() => onOpen(kind)}><Plus size={16} />{isEnglish ? 'Add' : 'Thêm'}</button>}</div>
