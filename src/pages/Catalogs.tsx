...
export function Catalogs() {
  ...
  const remove = async (kind: CatalogKind, item: CatalogItem) => {
    ...
    const confirmed = await askConfirm({
      title: isEnglish ? `Delete category “${displayName}”?` : `Xóa danh mục “${displayName}”?`,
      description: isEnglish
        ? 'Categories used by transactions cannot be deleted.'
        : 'Danh mục đã có giao dịch sẽ không thể xóa.',
      confirmLabel: isEnglish ? 'Delete category' : 'Xóa danh mục',
      danger: true,
    });
    if (!confirmed) return;
...
function Catalog({
  ...
  return <section className="catalog-card card p-4 sm:p-5">
    ...
    {isEditingHere && <form className="catalog-editor mb-3 space-y-2" onSubmit={onSubmit}>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-black/10 bg-gray-50 p-2 dark:border-white/10 dark:bg-white/[.04]">
          <span className="grid size-10 shrink-0 place-items-center rounded-lg bg-[var(--surface)] text-[var(--primary)] shadow-sm" aria-label={isEnglish ? `Selected icon: ${getCatalogIconLabel(icon)}` : `Biểu tượng đã chọn: ${getCatalogIconLabel(icon)}`}>
      ...
      {kind === 'purpose' && <label className="flex items-start gap-2 rounded-xl border border-black/10 bg-black/[.02] p-3 text-sm dark:border-white/10 dark:bg-white/[.03]">
        <input className="mt-0.5 size-4 accent-[var(--primary)]" type="checkbox" aria-label={isEnglish ? 'Track in budgets' : 'Theo dõi trong ngân sách'} checked={budgetEnabled} onChange={(event) => onBudgetEnabledChange(event.target.checked)} />
      ...
      <button type="submit" className="btn-primary w-full" disabled={saving}>{saving ? (isEnglish ? 'Saving…' : 'Đang lưu…') : editor?.id ? (isEnglish ? 'Save new name' : 'Lưu tên mới') : (isEnglish ? 'Save category' : 'Lưu danh mục')}</button>
    </form>}
    ...
    {items.length ? <div className="catalog-list divide-y divide-black/10 dark:divide-white/10">{items.map((item) => {
      ...
      return <div key={item.id} className="catalog-item flex items-center justify-between gap-2"><div className="flex min-w-0 flex-1 items-center gap-2.5"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[var(--primary-soft)] text-[var(--primary)]" title={getCatalogIconLabel(item.icon)}><ItemIcon size={17} aria-hidden="true" /></span><div className="min-w-0 flex-1"><span className="block whitespace-normal break-words lg:whitespace-nowrap" title={displayName}>{displayName}</span>{kind === 'purpose' && item.budgetEnabled === false && <span className="ui-chip mt-1 w-fit whitespace-nowrap text-[11px]">{isEnglish ? 'Budget hidden' : 'Ẩn ngân sách'}</span>}</div></div>{canManage && <div className="flex shrink-0 items-center gap-1"><button type="button" className="catalog-action icon-button text-[var(--primary)] hover:bg-[var(--primary-soft)] focus-visible:outline focus-visible:outline-2" aria-label={`${isEnglish ? 'Edit' : 'Sửa'} ${displayName}`} title={isEnglish ? 'Edit' : 'Sửa'} onClick={() => onOpen(kind, item)}><Pencil size={17} aria-hidden="true" /></button><button type="button" className="catalog-action icon-button text-red-600 hover:bg-red-50 focus-visible:outline focus-visible:outline-2 disabled:opacity-50 dark:text-red-300 dark:hover:bg-red-950/30" aria-label={`${isEnglish ? 'Delete' : 'Xóa'} ${displayName}`} title={isEnglish ? 'Delete' : 'Xóa'} disabled={deletingId === item.id} onClick={() => onDelete(kind, item)}><Trash2 size={17} aria-hidden="true" /></button></div>}</div>;
