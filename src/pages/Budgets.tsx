...
function statusClass(status: BudgetStatus) {
  if (status === 'warning') return 'border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-400/50 dark:bg-amber-950/30 dark:text-amber-200';
  if (status === 'over') return 'border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-400/50 dark:bg-rose-950/30 dark:text-rose-200';
  if (status === 'within') return 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-400/50 dark:bg-emerald-950/30 dark:text-emerald-200';
  return 'border-gray-200 bg-gray-50 text-gray-600 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300';
}

function progressClass(status: BudgetStatus) {
  if (status === 'over') return 'bg-rose-600';
  if (status === 'warning') return 'bg-amber-500';
  if (status === 'within') return 'bg-emerald-600';
  return 'bg-slate-500';
}
...
export function Budgets() {
  ...
  return <div className="budgets-page space-y-5">
    ...
    {summary && <>
      ...
      {summary.unbudgetedSpent > 0 && <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-400/50 dark:bg-amber-950/30 dark:text-amber-200">{en ? `${formatBudgetInput(summary.unbudgetedSpent)} ₫ was spent in purposes without a budget.` : `${formatBudgetInput(summary.unbudgetedSpent)} ₫ đã chi ở các mục đích chưa đặt ngân sách.`}</p>}
...
function SummaryCard({ label, value, tone }: { label: string; value: number; tone: 'violet' | 'rose' | 'emerald' | 'over' }) {
  const iconClass = tone === 'violet' ? 'bg-violet-100 text-violet-700 dark:bg-violet-950/40 dark:text-violet-200' : tone === 'rose' ? 'bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-200' : tone === 'over' ? 'bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-200' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-200';
