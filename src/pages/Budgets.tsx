import { AlertTriangle, CalendarDays, CheckCircle2, Copy, ExternalLink, Pencil, PiggyBank, Save, Trash2, X } from 'lucide-react';
...
function statusClass(status: BudgetStatus) {
  if (status === 'warning') return 'border-[var(--warning)] bg-[var(--warning-soft)] text-[var(--warning-strong)]';
  if (status === 'over') return 'border-[var(--danger)] bg-[var(--danger-soft)] text-[var(--danger-strong)]';
  if (status === 'within') return 'border-[var(--success)] bg-[var(--success-soft)] text-[var(--success-strong)]';
  return 'border-[var(--border)] bg-[var(--surface-muted)] text-[var(--muted)]';
}

function progressClass(status: BudgetStatus) {
  if (status === 'over') return 'bg-[var(--danger)]';
  if (status === 'warning') return 'bg-[var(--warning)]';
  if (status === 'within') return 'bg-[var(--success)]';
  return 'bg-[var(--muted)]';
}
...
export function Budgets() {
  ...
  if (isSupabaseConfigured && !familyId)
    return <p role="alert" className="rounded-xl bg-[var(--danger-soft)] p-4 text-[var(--danger-strong)]">{en ? 'No active family was found. Please reload and try again.' : 'Không tìm thấy gia đình đang hoạt động. Vui lòng tải lại rồi thử lại.'}</p>;
  ...
  return <div className="budgets-page space-y-5">
    ...
    {summary && <>
      ...
      {summary.unbudgetedSpent > 0 && <p className="rounded-xl border border-[var(--warning)] bg-[var(--warning-soft)] p-3 text-sm text-[var(--warning-strong)]">{en ? <><Amount value={summary.unbudgetedSpent} /> was spent in purposes without a budget.</> : <><Amount value={summary.unbudgetedSpent} /> đã chi ở các mục đích chưa đặt ngân sách.</>}</p>}
...
function SummaryCard({ label, value, tone }: { label: string; value: number; tone: 'violet' | 'rose' | 'emerald' | 'over' }) {
  const iconClass = tone === 'violet' ? 'bg-[var(--accent-soft)] text-[var(--accent-strong)]' : tone === 'rose' || tone === 'over' ? 'bg-[var(--danger-soft)] text-[var(--danger-strong)]' : 'bg-[var(--success-soft)] text-[var(--success-strong)]';
  return <div className="card budget-summary-card p-3 sm:p-4"><span className={`mb-2 inline-flex size-8 items-center justify-center rounded-xl ${iconClass}`}><PiggyBank size={16} aria-hidden="true" /></span><p className="text-xs font-semibold text-gray-500 dark:text-gray-400">{label}</p><Amount value={value} className="mt-1 block break-words text-base font-extrabold sm:text-lg" label={label} /></div>;
}
...
function BudgetRow({ item, year, month, en, canManage, editing, editorAmount, editorThreshold, editorError, saving, deleting, onBeginEdit, onCloseEdit, onAmountChange, onThresholdChange, onSubmit, onDelete }: BudgetRowProps) {
  ...
  return <article className="budget-row p-4 sm:p-5">
    <div className="flex flex-col gap-3 lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(220px,1.25fr)_auto] lg:items-center lg:gap-5">
      <div className="min-w-0"><div className="flex items-start gap-3"><span className={`mt-0.5 grid size-9 shrink-0 place-items-center rounded-xl ${item.status === 'over' ? 'bg-[var(--danger-soft)] text-[var(--danger-strong)]' : item.status === 'warning' ? 'bg-[var(--warning-soft)] text-[var(--warning-strong)]' : 'bg-[var(--success-soft)] text-[var(--success-strong)]'}`}><PiggyBank size={18} aria-hidden="true" /></span><div className="min-w-0"><h4 className="truncate font-bold" title={displayName}>{displayName}</h4><span className={`mt-1 inline-flex rounded-full border px-2 py-0.5 text-xs font-bold ${statusClass(item.status)}`}>{item.status === 'over' ? <AlertTriangle size={13} className="mr-1" aria-hidden="true" /> : item.status === 'within' ? <CheckCircle2 size={13} className="mr-1" aria-hidden="true" /> : null}{statusLabel(item.status, en)}</span></div></div></div>
      <div className="min-w-0"><div className="flex items-end justify-between gap-3 text-sm"><span className="min-w-0 truncate text-gray-600 dark:text-gray-300">{item.budget === null ? (en ? 'No budget set' : 'Chưa đặt ngân sách') : <><strong>{formatBudgetInput(item.spent)} ₫</strong><span className="text-gray-500 dark:text-gray-400"> / {formatBudgetInput(item.budget)} ₫</span></>}</span><span className="shrink-0 font-bold text-gray-700 dark:text-gray-200">{item.usagePercent === null ? '—' : `${item.usagePercent.toLocaleString(en ? 'en-US' : 'vi-VN', { maximumFractionDigits: 1 })}%`}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-slate-800" role="progressbar" aria-label={en ? `${displayName} budget usage` : `Mức sử dụng ngân sách ${displayName}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={usage} aria-valuetext={item.budget === null ? (en ? 'No budget set' : 'Chưa đặt ngân sách') : `${item.usagePercent?.toLocaleString(en ? 'en-US' : 'vi-VN', { maximumFractionDigits: 1 }) || 0}%`}><div className={`h-full rounded-full transition-all ${progressClass(item.status)}`} style={{ width: `${usage}%` }} /></div><p className="mt-2 text-xs text-gray-500 dark:text-gray-400">{item.budget === null ? (en ? `${formatBudgetInput(item.spent)} ₫ spent without a budget` : `Đã chi ${formatBudgetInput(item.spent)} ₫ nhưng chưa đặt ngân sách`) : item.remaining! < 0 ? (en ? `${formatBudgetInput(Math.abs(item.remaining!))} ₫ over` : `Vượt ${formatBudgetInput(Math.abs(item.remaining!))} ₫`) : (en ? `${formatBudgetInput(item.remaining!)} ₫ remaining` : `Còn ${formatBudgetInput(item.remaining!)} ₫`)}</p></div>
      <div className="flex flex-wrap items-center gap-2 lg:justify-end"><Link className="btn-secondary inline-flex items-center gap-1.5 text-sm" to={filterLink}><ExternalLink size={15} aria-hidden="true" />{en ? 'Transactions' : 'Giao dịch'}</Link>{canManage && <button type="button" className="asset-action-button asset-icon-action btn-secondary inline-flex items-center justify-center" aria-label={item.budget === null ? (en ? 'Set budget' : 'Đặt ngân sách') : (en ? 'Edit budget' : 'Sửa ngân sách')} title={item.budget === null ? (en ? 'Set budget' : 'Đặt ngân sách') : (en ? 'Edit budget' : 'Sửa ngân sách')} onClick={onBeginEdit}><Pencil size={18} aria-hidden="true" /></button>}{canManage && item.budgetId && <button type="button" className="asset-action-button asset-icon-action danger-button inline-flex items-center justify-center" aria-label={en ? `Delete ${displayName} budget` : `Xóa ngân sách ${displayName}`} title={en ? 'Delete budget' : 'Xóa ngân sách'} disabled={deleting} onClick={onDelete}><Trash2 size={18} aria-hidden="true" /></button>}</div>
    </div>
    {editing && <form className="budget-editor mt-4 rounded-2xl border border-black/10 bg-[var(--surface-muted)] p-3 dark:border-white/10" onSubmit={(event) => onSubmit(event, item.purposeId)}><div className="mb-3 flex items-center justify-between gap-3"><div><h5 className="font-bold">{item.budget === null ? (en ? 'Set monthly budget' : 'Đặt ngân sách tháng') : (en ? 'Edit monthly budget' : 'Sửa ngân sách tháng')}</h5><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{en ? 'Only actual expenses are counted.' : 'Chỉ chi tiêu thực tế được tính.'}</p></div><button type="button" className="icon-button" aria-label={en ? 'Close budget editor' : 'Đóng trình sửa ngân sách'} onClick={onCloseEdit}><X size={18} /></button></div><div className="grid gap-3 sm:grid-cols-[minmax(0,1.4fr)_minmax(150px,.6fr)]"><label><span className="label">{en ? 'Monthly amount (VND)' : 'Số tiền tháng (VND)'}</span><input className="field" type="text" inputMode="numeric" autoFocus required value={editorAmount} onChange={(event) => { const digits = event.target.value.replace(/\D/g, ''); onAmountChange(digits ? formatBudgetInput(Number(digits)) : ''); }} placeholder="15.000.000" aria-label={en ? `Budget amount for ${displayName}` : `Số tiền ngân sách ${displayName}`} /></label><label><span className="label">{en ? 'Warn at (%)' : 'Cảnh báo ở (%)'}</span><input className="field" type="number" min="1" max="100" step="1" required value={editorThreshold} onChange={(event) => onThresholdChange(event.target.value)} aria-label={en ? `Warning threshold for ${displayName}` : `Ngưỡng cảnh báo ${displayName}`} /></label></div>{editorError && <p role="alert" className="mt-3 rounded-lg border border-[var(--danger)] bg-[var(--danger-soft)] p-2 text-sm text-[var(--danger-strong)]">{editorError}</p>}<div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:justify-end"><button type="button" className="btn-secondary inline-flex items-center justify-center gap-1.5" onClick={onCloseEdit}><X size={15} aria-hidden="true" />{en ? 'Cancel' : 'Hủy'}</button><button type="submit" className="btn-primary inline-flex items-center justify-center gap-1.5" disabled={saving}><Save size={15} aria-hidden="true" />{saving ? (en ? 'Saving…' : 'Lưu ngân sách') : (en ? 'Save budget' : 'Lưu ngân sách')}</button></div></form>}
  </article>;
}
