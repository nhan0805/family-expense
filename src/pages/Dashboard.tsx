...
function Kpi({ label, value, icon: Icon, tone, meta, to }: { label: string; value: number; icon: LucideIcon; tone: Tone; meta: string; to: string }) {
  ...
  return <Link to={to} className="card min-w-0 p-3 transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#137050] sm:p-4" aria-label={`Mở giao dịch theo ${label}`}><div className="flex items-start gap-2"><span className={`grid size-9 shrink-0 place-items-center rounded-xl ${toneClass}`}><Icon size={18} aria-hidden="true" /></span><div className="min-w-0 flex-1"><p className="min-h-8 text-xs font-semibold leading-4 text-gray-500 dark:text-gray-400">{label}</p><p className="mt-1 whitespace-nowrap text-xl font-extrabold sm:text-2xl" title={formatVnd(value)} aria-label={`${label}: ${formatVnd(value)}`}>{formatCompactVnd(value)}</p></div></div><p className="mt-3 truncate text-xs text-gray-500 dark:text-gray-400" title={meta}>{meta}</p></Link>;
}
