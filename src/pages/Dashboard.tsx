...
function AssetSnapshot({ summary, en, loading }: { summary?: AssetSummary; en: boolean; loading: boolean }) {
  ...
  return <section className="card overflow-hidden p-4 sm:p-5" aria-labelledby="dashboard-assets-title">
    ...
    {(summary.savings.length || summary.gold.length) ? <div className="mt-4 grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-black/10 p-3 dark:border-white/10"><div className="flex items-center justify-between gap-2"><h4 id="dashboard-gold-list-title" className="flex min-w-0 items-center gap-2 font-bold"><Crown size={16} aria-hidden="true" />{en ? 'Gold' : 'Danh sách vàng'}</h4><div className="flex shrink-0 items-center gap-2"><span className={goldPnl === null || goldPnl >= 0 ? 'text-xs font-bold text-emerald-700 dark:text-emerald-300' : 'text-xs font-bold text-rose-700 dark:text-rose-300'}>{en ? 'P/L' : 'Lãi/lỗ'} {goldPnl === null ? '—' : formatVnd(goldPnl)}</span>{summary.gold.length > 0 && <button type="button" className="btn-secondary inline-flex min-h-11 items-center justify-center gap-1.5 px-3 py-1.5 text-xs" aria-expanded={showGoldList} aria-controls="dashboard-gold-list" aria-label={showGoldList ? (en ? 'Collapse gold list' : 'Thu gọn danh sách vàng') : (en ? `Show gold list (${summary.gold.length})` : `Mở danh sách vàng (${summary.gold.length})`)} onClick={() => setShowGoldList((value) => !value)}><span>{showGoldList ? (en ? 'Collapse' : 'Thu gọn') : (en ? 'Show list' : 'Xem danh sách')}</span><ChevronDown size={15} aria-hidden="true" className={`transition-transform ${showGoldList ? 'rotate-180' : ''}`} /></button>}</div></div>{summary.gold.length ? <ul id="dashboard-gold-list" aria-labelledby="dashboard-gold-list-title" hidden={!showGoldList} className="mt-2 divide-y divide-black/10 text-sm dark:divide-white/10">{summary.gold.map((asset) => <li key={asset.id} className="flex flex-wrap items-center justify-between gap-2 py-2"><span><span className="block font-semibold">{asset.remainingQuantityChi.toLocaleString('vi-VN', { maximumFractionDigits: 3 })} {en ? 'mace' : 'chỉ'}</span><span className="text-xs text-gray-500">{en ? 'Bought' : 'Ngày mua'} {formatDate(asset.purchaseDate)} · {formatVnd(asset.purchasePricePerChi)}/{en ? 'mace' : 'chỉ'}</span></span><span className="font-bold tabular-nums">{asset.estimatedSellPricePerChi === null ? '—' : formatVnd(Math.round(asset.remainingQuantityChi * asset.estimatedSellPricePerChi))}</span></li>)}</ul> : <p className="mt-2 text-sm text-gray-500">{en ? 'No gold yet.' : 'Chưa có vàng đang giữ.'}</p>}</div>
