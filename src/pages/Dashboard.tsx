...
import { buildLocalAssetSummary, expectedSavingsInterest, expectedSavingsInterestToDate, summarizeGoldHoldings, type AssetSummary } from '../lib/assets';
...
import {
  ...
  REMOTE_DASHBOARD_REFRESH_INTERVAL_MS,
...
type TrendSeriesKey = 'expense' | 'income' | 'net';

const trendSeriesMeta: Record<TrendSeriesKey, { vi: string; en: string; color: string }> = {
  expense: { vi: 'Chi tiêu', en: 'Expenses', color: 'var(--chart-expense)' },
  income: { vi: 'Thu nhập', en: 'Income', color: 'var(--chart-income)' },
  net: { vi: 'Thu ròng', en: 'Net value', color: 'var(--chart-net)' },
};
...
export function Dashboard() {
  ...
  const [hiddenTrendSeries, setHiddenTrendSeries] = useState<TrendSeriesKey[]>([]);
  const toggleTrendSeries = (series: TrendSeriesKey) => {
    setHiddenTrendSeries((current) => current.includes(series)
  ...
  const dashboardQuery = useQuery({
    ...
    refetchInterval: isSupabaseConfigured && Boolean(familyId) && validRange
      ? REMOTE_DASHBOARD_REFRESH_INTERVAL_MS
      : false,
    staleTime: REMOTE_DASHBOARD_REFRESH_INTERVAL_MS,
  ...
  const trendHasData = trend.some((item) => item.expense || item.income);
  const trendSummary = trendHasData
    ? trend.map((item) => `${formatPeriodKey(item.key, en)}: ${en ? 'expenses' : 'chi'} ${formatVnd(item.expense)}, ${en ? 'income' : 'thu'} ${formatVnd(item.income)}`).join('; ')
    : (en ? 'No trend data for this period.' : 'Chưa có dữ liệu xu hướng trong kỳ này.');
  ...
  return (
    <div className="dashboard-page space-y-5">
      ...
      <section className={`ui-stagger dashboard-kpi-grid grid grid-cols-2 gap-3 sm:gap-4 ${hasMultiMonthView ? 'xl:grid-cols-6' : 'xl:grid-cols-3'}`} aria-label={en ? 'Financial summary' : 'Tóm tắt tài chính'}>
        <Kpi en={en} label={en ? 'Total income' : 'Tổng thu'} value={selectedIncome} icon={ArrowDownToLine} tone="emerald" meta={renderChange(incomeChange, en, 'vs previous period')} to={periodFilterLink('Thu nhập')} />
        <Kpi en={en} label={en ? 'Total expenses' : 'Tổng chi'} value={selectedExpense} icon={ArrowUpFromLine} tone="rose" meta={renderChange(expenseChange, en, 'vs previous period')} to={periodFilterLink('Chi tiêu')} />
        <div className={hasMultiMonthView ? 'min-w-0' : 'col-span-2 min-w-0 xl:col-span-1'}><Kpi en={en} label={en ? 'Net value' : 'Giá trị ròng'} value={selectedIncome - selectedExpense} icon={Scale} tone={selectedIncome >= selectedExpense ? 'emerald' : 'rose'} meta={en ? 'Income minus expenses' : 'Thu nhập trừ chi tiêu'} to={periodFilterLink()} /></div>
        {hasMultiMonthView && <>
          <Kpi en={en} label={en ? 'Average / month' : 'Trung bình / tháng'} value={averageExpense} icon={BarChart3} tone="sky" meta={en ? `${periodCount} mo.` : `${periodCount} tháng`} to={periodFilterLink('Chi tiêu')} />
          <Kpi en={en} label={en ? 'Peak month' : 'Tháng cao nhất'} value={highestMonth.expense} icon={TrendingUp} tone="violet" meta={highestMonth.expense > 0 ? `T${highestMonth.key.slice(5, 7)}/${highestMonth.key.slice(0, 4)}` : (en ? 'No data' : 'Chưa có dữ liệu')} to={periodFilterLink('Chi tiêu', highestMonth.expense > 0 ? rangeForPeriods([{ key: highestMonth.key, label: highestMonth.label }]) : selectedRange)} />
          <Kpi en={en} label={en ? 'Lowest month' : 'Tháng thấp nhất'} value={lowestMonth.expense} icon={TrendingDown} tone="sky" meta={lowestMonth.expense > 0 ? `T${lowestMonth.key.slice(5, 7)}/${lowestMonth.key.slice(0, 4)}` : (en ? 'No data' : 'Chưa có dữ liệu')} to={periodFilterLink('Chi tiêu', lowestMonth.expense > 0 ? rangeForPeriods([{ key: lowestMonth.key, label: lowestMonth.label }]) : selectedRange)} />
        </>}
      </section>
      ...
      <details className="group card dashboard-breakdown-card min-w-0 overflow-hidden">
        <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 p-4 sm:p-5 [&::-webkit-details-marker]:hidden">
          <div className="min-w-0">
            <h3 className="text-lg font-bold">{en ? 'Top categories over time' : 'Top danh mục theo thời gian'}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">{en ? 'Click a row to inspect the filtered transactions.' : 'Bấm vào một dòng để xem các giao dịch đã lọc.'}</p>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <span className="ui-chip">{topCategories.length ? `${topCategories.length} ${en ? 'categories' : 'danh mục'}` : (en ? 'No data' : 'Chưa có dữ liệu')}</span>
            <TrendingUp className="text-[var(--chart-net)]" size={22} aria-hidden="true" />
            <ChevronDown size={18} aria-hidden="true" className="transition-transform group-open:rotate-180" />
          </div>
        </summary>
        <div className="px-4 pb-4 sm:px-5 sm:pb-5">
          {topCategories.length ? <div className="space-y-2">{topCategories.map((item) => <Link key={item.id} to={`${periodFilterLink(undefined)}&expenseTypeId=${encodeURIComponent(item.id)}`} className="card-interactive grid grid-cols-[minmax(0,1fr)_minmax(120px,1.6fr)_auto] items-center gap-3 rounded-xl border border-black/5 p-3 hover:bg-black/[.025] focus-visible:outline focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] dark:border-white/10 dark:hover:bg-white/5"><div className="min-w-0"><p className="break-words font-semibold [overflow-wrap:anywhere]">{item.name}</p><p className="text-xs text-gray-500">{formatVnd(item.value)}</p></div><MiniTrend values={item.trend} label={`${item.name}: ${formatVnd(item.value)}`} /><ChangeBadge value={changePercent(item.value, item.previousValue)} en={en} /></Link>)}</div> : <div className="rounded-xl bg-black/[.025] px-4 py-7 text-center text-sm text-gray-500 dark:bg-white/[.04]">{en ? 'No category data in this period.' : 'Chưa có dữ liệu danh mục trong kỳ này.'}</div>}
        </div>
      </details>

      <details className="group card dashboard-details overflow-hidden p-4 sm:p-5">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 font-bold [&::-webkit-details-marker]:hidden">
          <span>{en ? 'Income breakdown' : 'Phân bổ thu nhập'}</span>
          <ChevronDown size={18} aria-hidden="true" className="transition-transform group-open:rotate-180" />
        </summary>
        <div className="ui-stagger mt-4 grid gap-4 lg:grid-cols-2"><div className="card dashboard-pie-card min-w-0 overflow-hidden p-4 sm:p-5"><ExpensePieChart title={en ? 'Income by purpose' : 'Thu nhập theo mục đích'} data={incomeByPurpose} to={periodFilterLink('Thu nhập')} filterKey="purposeId" en={en} income /></div><div className="card dashboard-pie-card min-w-0 overflow-hidden p-4 sm:p-5"><ExpensePieChart title={en ? 'Income by category' : 'Thu nhập theo danh mục'} data={incomeByExpenseType} to={periodFilterLink('Thu nhập')} filterKey="expenseTypeId" en={en} income /></div></div>
      </details>

      <details className="group card attention-card overflow-hidden border-amber-200 bg-amber-50/60 dark:border-amber-900/60 dark:bg-amber-950/20" aria-labelledby="dashboard-insights-title">
        <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 p-4 sm:p-5 [&::-webkit-details-marker]:hidden">
          <div className="flex min-w-0 items-center gap-2">
            <Sparkles size={19} className="shrink-0 text-amber-600" aria-hidden="true" />
            <div className="min-w-0">
              <h3 id="dashboard-insights-title" className="font-bold">{en ? 'What stands out' : 'Điểm đáng chú ý'}</h3>
              <p className="text-xs text-gray-600 dark:text-gray-300">{insights.length ? `${insights.length} ${en ? 'insights available' : 'nhận xét sẵn có'}` : (en ? 'Open to see insights and AI summary.' : 'Mở để xem nhận xét và tóm tắt AI.')}</p>
            </div>
          </div>
          <ChevronDown size={18} aria-hidden="true" className="shrink-0 transition-transform group-open:rotate-180" />
        </summary>
        <div className="px-4 pb-4 sm:px-5 sm:pb-5">
          <div className="mb-3 flex justify-end"><button type="button" className="btn-secondary inline-flex shrink-0 items-center gap-2 text-sm" disabled={!isSupabaseConfigured || !validRange || aiSummaryBusy} onClick={() => void runAiSummary()} title={!isSupabaseConfigured ? (en ? 'Connect Supabase to use AI' : 'Cần kết nối Supabase để dùng AI') : undefined}><Sparkles size={16} aria-hidden="true" />{aiSummaryBusy ? (en ? 'Summarizing…' : 'Đang tóm tắt…') : aiSummaryQuery.isError ? (en ? 'Retry AI summary' : 'Thử lại tóm tắt AI') : (en ? 'Summarize with AI' : 'Tóm tắt bằng AI')}</button></div>
          {insights.length ? <ul className="grid gap-2 text-sm sm:grid-cols-2">{insights.map((insight) => <li key={insight} className="rounded-xl border border-amber-100 bg-white/70 p-3 dark:border-amber-900/40 dark:bg-white/5">{insight}</li>)}</ul> : <p className="text-sm text-gray-600 dark:text-gray-300">{en ? 'Insights will appear when there is enough actual data.' : 'Nhận xét sẽ xuất hiện khi có đủ dữ liệu giao dịch thực tế.'}</p>}
          {aiSummaryQuery.isError && <div role="alert" className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-lg bg-red-100/70 p-3 text-sm text-red-700 dark:bg-red-950/30 dark:text-red-300"><span>{aiErrorMessage(aiSummaryQuery.error, en, 'summary')}</span><button type="button" className="btn-secondary px-3 py-1.5 text-xs" onClick={() => void runAiSummary()}>{en ? 'Retry' : 'Thử lại'}</button></div>}
          {aiSummaryQuery.data && <div className="mt-3 rounded-xl border border-amber-200 bg-white/75 p-3 text-sm dark:border-amber-900/60 dark:bg-white/5"><p className="leading-6">{aiSummaryQuery.data.summary}</p>{aiSummaryQuery.data.highlights.length > 0 && <ul className="mt-2 list-disc space-y-1 pl-5 text-gray-700 dark:text-gray-300">{aiSummaryQuery.data.highlights.map((highlight) => <li key={highlight}>{highlight}</li>)}</ul>}</div>}
        </div>
      </details>
    </div>
  );
...
function AssetSnapshot({ summary, en, loading }: { summary?: AssetSummary; en: boolean; loading: boolean }) {
  ...
  const goldHolding = summarizeGoldHoldings(summary.gold, summary.goldBuybackPricePerChi);
  const goldPnl = goldHolding.unrealizedPnl;
  ...
  return <section className="card overflow-hidden p-4 sm:p-5" aria-labelledby="dashboard-assets-title">
    ...
    <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="min-w-0 rounded-2xl bg-amber-50 p-4 text-amber-900 dark:bg-amber-950/20 dark:text-amber-100"><div className="flex items-center gap-2 text-sm font-semibold"><Crown size={17} aria-hidden="true" />{en ? 'Gold estimate' : 'Vàng ước tính'}</div><p className="mt-2 break-words text-xl font-extrabold tabular-nums">{formatVnd(summary.goldEstimatedTotal)}</p><p className="mt-1 text-xs opacity-75">{quantityLabel}</p><p className="mt-1 text-xs opacity-75">{en ? 'Avg. cost' : 'Giá vốn bình quân'} {averageCostLabel}{summary.goldMissingEstimateCount ? ` · ${en ? 'missing price' : 'chưa có giá'}` : ''}</p><p className={`mt-1 inline-flex max-w-full items-center gap-1 text-xs font-semibold ${goldPnlClass}`}><GoldPnlIcon size={14} aria-hidden="true" /><span>{en ? 'Est. P/L' : 'Lãi/lỗ ước tính'} {goldPnl === null ? '—' : formatSignedVnd(goldPnl)}</span></p></div>
    </div>
    {(summary.savings.length || summary.gold.length) ? <div className="mt-4 grid gap-4 lg:grid-cols-2">
      <div className="rounded-2xl border border-black/10 p-3 dark:border-white/10"><div className="flex items-center justify-between gap-2"><h4 id="dashboard-savings-list-title" className="flex min-w-0 items-center gap-2 font-bold"><Landmark size={22} className="shrink-0" aria-hidden="true" />{en ? 'Savings books' : 'Sổ tiết kiệm'}</h4>{summary.savings.length > 0 && <button type="button" className="btn-secondary inline-flex min-h-11 shrink-0 items-center justify-center gap-1.5 px-3 py-1.5 text-xs" aria-expanded={showSavingsList} aria-controls="dashboard-savings-list" aria-label={showSavingsList ? (en ? 'Collapse savings books list' : 'Thu gọn danh sách sổ tiết kiệm') : (en ? `Show savings books list (${summary.savings.length})` : `Mở danh sách sổ tiết kiệm (${summary.savings.length})`)} onClick={() => setShowSavingsList((value) => !value)}><span>{showSavingsList ? (en ? 'Collapse' : 'Thu gọn') : (en ? 'Show list' : 'Xem danh sách')}</span><ChevronDown size={15} aria-hidden="true" className={`transition-transform ${showSavingsList ? 'rotate-180' : ''}`} /></button>}</div>{summary.savings.length ? <ul id="dashboard-savings-list" aria-labelledby="dashboard-savings-list-title" hidden={!showSavingsList} className="mt-2 divide-y divide-black/10 text-sm dark:divide-white/10">{summary.savings.map((account) => <li key={account.id} className="flex flex-wrap items-center justify-between gap-2 py-2"><span className="min-w-0"><span className="block break-words font-semibold [overflow-wrap:anywhere]">{account.bankName} · {account.name}</span><span className="text-xs text-gray-500">{en ? 'Maturity' : 'Đáo hạn'} {formatDate(account.maturityOn)}</span></span><span className="font-bold tabular-nums">{formatVnd(account.currentBalance)}</span></li>)}</ul> : <p className="mt-2 text-sm text-gray-500">{en ? 'No active books.' : 'Chưa có sổ đang theo dõi.'}</p>}</div>
      <div className="rounded-2xl border border-black/10 p-3 dark:border-white/10"><div className="flex items-center justify-between gap-2"><h4 id="dashboard-gold-list-title" className="flex min-w-0 items-center gap-2 font-bold"><Crown size={22} className="shrink-0" aria-hidden="true" />{en ? 'Gold' : 'Vàng'}</h4><div className="flex shrink-0 items-center gap-2">{summary.gold.length > 0 && <button type="button" className="btn-secondary inline-flex min-h-11 items-center justify-center gap-1.5 px-3 py-1.5 text-xs" aria-expanded={showGoldList} aria-controls="dashboard-gold-list" aria-label={showGoldList ? (en ? 'Collapse gold list' : 'Thu gọn danh sách vàng') : (en ? `Show gold list (${summary.gold.length})` : `Mở danh sách vàng (${summary.gold.length})`)} onClick={() => setShowGoldList((value) => !value)}><span>{showGoldList ? (en ? 'Collapse' : 'Thu gọn') : (en ? 'Show list' : 'Xem danh sách')}</span><ChevronDown size={15} aria-hidden="true" className={`transition-transform ${showGoldList ? 'rotate-180' : ''}`} /></button>}</div></div>{summary.gold.length ? <ul id="dashboard-gold-list" aria-labelledby="dashboard-gold-list-title" hidden={!showGoldList} className="mt-2 divide-y divide-black/10 text-sm dark:divide-white/10">{summary.gold.map((asset) => <li key={asset.id} className="flex flex-wrap items-center justify-between gap-2 py-2"><span><span className="block font-semibold">{asset.remainingQuantityChi.toLocaleString('vi-VN', { maximumFractionDigits: 3 })} {en ? 'mace' : 'chỉ'}</span><span className="text-xs text-gray-500">{en ? 'Bought' : 'Ngày mua'} {formatDate(asset.purchaseDate)} · {formatVnd(asset.purchasePricePerChi)}/{en ? 'mace' : 'chỉ'}</span></span><span className="font-bold tabular-nums">{asset.estimatedSellPricePerChi === null ? '—' : formatVnd(Math.round(asset.remainingQuantityChi * asset.estimatedSellPricePerChi))}</span></li>)}</ul> : <p className="mt-2 text-sm text-gray-500">{en ? 'No gold yet.' : 'Chưa có vàng đang giữ.'}</p>}</div>
    </div> : <p className="mt-4 rounded-xl bg-black/[.025] p-3 text-sm text-gray-500 dark:bg-white/[.04]">{en ? 'Add a savings book or gold to see the asset list here.' : 'Thêm sổ tiết kiệm hoặc vàng để xem danh sách tài sản tại đây.'}</p>}
  </section>;
}
...
function TrendLegend({ en, hiddenSeries, onToggle }: { en: boolean; hiddenSeries: TrendSeriesKey[]; onToggle: (series: TrendSeriesKey) => void }) {
  return <div className="flex flex-wrap justify-end gap-x-2 gap-y-1 text-xs" role="group" aria-label={en ? 'Trend series' : 'Các loại dữ liệu xu hướng'}>{(Object.keys(trendSeriesMeta) as TrendSeriesKey[]).map((series) => {
    const label = trendSeriesMeta[series][en ? 'en' : 'vi'];
    const hidden = hiddenSeries.includes(series);
    return <button key={series} type="button" className={`flex min-h-11 items-center gap-1.5 rounded-lg px-2 transition-opacity hover:bg-black/5 focus-visible:outline focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] dark:hover:bg-white/5 ${hidden ? 'opacity-50' : ''}`} aria-pressed={!hidden} aria-label={`${hidden ? (en ? 'Show' : 'Hiện') : (en ? 'Hide' : 'Ẩn')} ${label}`} onClick={() => onToggle(series)}><span className="size-2.5 shrink-0 rounded-full" style={{ backgroundColor: trendSeriesMeta[series].color }} aria-hidden="true" /><span className={hidden ? 'line-through' : undefined}>{label}</span></button>;
  })}</div>;
}
...
function ExpensePieChart({ title, data, to, filterKey, en, income = false }: { title: string; data: ExpenseChartItem[]; to: string; filterKey: 'purposeId' | 'expenseTypeId'; en: boolean; income?: boolean }) {
  ...
  const tableId = `dashboard-${income ? 'income' : 'expense'}-${filterKey}-pie-table`;
  ...
  return <>
    <div className="flex items-start justify-between gap-3"><h3 className="font-bold">{title}</h3>{pieData.length > 0 && <button type="button" className="btn-secondary shrink-0 px-2.5 py-1.5 text-xs" aria-expanded={showTable} aria-controls={tableId} onClick={() => setShowTable((value) => !value)}>{showTable ? (en ? 'Hide table' : 'Ẩn bảng') : (en ? 'View table' : 'Xem dạng bảng')}</button>}</div>
    <div className="min-h-72 min-w-0 max-w-full pt-3" role="group" aria-label={title} aria-describedby={summaryId}>
      <p id={summaryId} className="sr-only">{pieData.length ? (en ? `${title}: ${formatVnd(total)} across ${pieData.length} categories. Use the table for exact values.` : `${title}: ${formatVnd(total)} qua ${pieData.length} danh mục. Mở bảng để đọc giá trị chính xác.`) : (en ? `No data for ${title}.` : `Chưa có dữ liệu cho ${title}.`)}</p>
      {pieData.length ? <>
        ...
        <ul className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs sm:grid-cols-3" aria-label={`${title} legend`}>{pieData.map((item) => { const interactive = item.id !== 'uncategorized' && (item.isOther ? Boolean(item.hiddenItems?.length) : true); return <li key={item.id || item.name} className="min-w-0"><button type="button" className="flex min-h-11 w-full min-w-0 items-center gap-1.5 rounded-lg px-1 text-left hover:bg-black/5 disabled:cursor-default disabled:hover:bg-transparent dark:hover:bg-white/5 dark:disabled:hover:bg-transparent" disabled={!interactive} onClick={() => openItem(item)} aria-label={`${item.name}: ${formatVnd(item.value)}`}><span className="size-2.5 shrink-0 rounded-full" style={{ backgroundColor: item.fill }} aria-hidden="true" /><span className="min-w-0 break-words leading-tight [overflow-wrap:anywhere]" title={`${item.name}: ${formatVnd(item.value)}`}>{item.name}</span></button></li>; })}</ul>
...
function Kpi({ en, label, value, icon: Icon, tone, meta, to }: { en: boolean; label: string; value: number; icon: LucideIcon; tone: Tone; meta: ReactNode; to: string }) {
  const toneClass = `kpi-tone-${tone}`;
  return <Link to={to} className="card card-interactive kpi-card group block h-full min-w-0 p-3 transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] sm:p-4" aria-label={`${en ? 'Open transactions filtered by' : 'Mở giao dịch theo'} ${label}`}><div className="min-w-0"><div className="flex items-center gap-2"><span className={`kpi-icon grid size-9 shrink-0 place-items-center rounded-xl ${toneClass}`}><Icon size={18} aria-hidden="true" /></span><p className="min-h-8 min-w-0 flex-1 text-xs font-semibold leading-4 text-gray-500 dark:text-gray-400">{label}</p></div><Amount value={value} variant="compact" className="kpi-value mt-2 block max-w-full break-words whitespace-normal text-lg font-extrabold leading-tight sm:text-xl" label={label} /></div><p className="kpi-meta mt-3 flex min-w-0 items-center gap-1 truncate text-xs leading-4">{meta}</p></Link>;
}
