import { BookOpen, Coins, House, LogOut, Menu, MoreHorizontal, PiggyBank, Plus, Repeat2, Settings2, Tags, UserRound, UsersRound, WalletCards, WifiOff, X } from 'lucide-react';
...
const links = [['/', 'overview', House], ['/giao-dich', 'transactions', WalletCards], ['/tai-san', 'assets', Coins], ['/ngan-sach', 'budgets', PiggyBank], ['/chi-phi-dinh-ky', 'recurringExpenses', Repeat2], ['/danh-muc', 'catalogs', Tags], ['/thanh-vien', 'members', UsersRound], ['/du-lieu', 'data', BookOpen]] as const;
...
export function Layout() {
  ...
  return <div className="app-shell">
    ...
    <header className="app-header sticky top-0 z-30 border-b px-4 py-3 backdrop-blur">
      <div className="app-header-inner mx-auto flex max-w-7xl items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <button className="hidden items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold hover:bg-black/5 dark:hover:bg-white/5 md:flex" disabled={signingOut} onClick={signOut}><LogOut size={18} aria-hidden="true"/>{signingOut ? t('loggingOut') : t('logout')}</button>
          <button ref={menuTriggerRef} className="icon-button md:hidden" aria-label={en ? 'Open menu' : 'Mở trình đơn'} onClick={() => setOpen(true)}><Menu aria-hidden="true" /></button>
        </div>
      </div>
    </header>
    ...
    <div className="mx-auto flex max-w-7xl">
      ...
      <aside ref={drawerRef} className={`app-sidebar ${open ? 'app-sidebar-open ui-drawer-enter fixed inset-y-0 left-0 z-50 flex' : menuMounted ? 'ui-drawer-exit fixed inset-y-0 left-0 z-50 flex' : 'hidden'} w-72 flex-col border-r p-4 md:sticky md:top-[68px] md:flex md:h-[calc(100vh-68px)] md:w-64`}>
        <div className="mb-3 flex items-center justify-between md:hidden">
          ...
          <button className="icon-button" aria-label={en ? 'Close menu' : 'Đóng trình đơn'} onClick={() => setOpen(false)}><X aria-hidden="true" /></button>
        </div>
        <nav className="mt-1 space-y-1 md:mt-2">{links.map(([to, label, Icon]) => <NavLink key={to} to={to} end={to === '/'} onClick={() => setOpen(false)} className={({ isActive }) => `app-nav-link flex items-center gap-3 rounded-xl px-3 py-3 font-semibold ${isActive ? 'app-nav-link-active' : ''}`}><Icon size={19} aria-hidden="true" />{t(label)}</NavLink>)}<NavLink to="/cai-dat" end onClick={() => setOpen(false)} className={({ isActive }) => `app-nav-link flex items-center gap-3 rounded-xl px-3 py-3 font-semibold ${isActive ? 'app-nav-link-active' : ''}`}><Settings2 size={19} aria-hidden="true" />{en ? 'Settings' : 'Cài đặt'}</NavLink><NavLink to="/cai-dat/giao-dich" end onClick={() => setOpen(false)} className={({ isActive }) => `app-nav-link flex items-center gap-3 rounded-xl px-3 py-3 font-semibold ${isActive ? 'app-nav-link-active' : ''}`}><Settings2 size={19} aria-hidden="true" />{en ? 'Default filters' : 'Bộ lọc mặc định'}</NavLink></nav>
        <div className="app-sidebar-footer mt-auto space-y-2 border-t pt-4">
          <ThemeSelect/>
          <button className="flex w-full items-center gap-3 rounded-xl px-3 py-3 font-medium text-red-700 hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-950/30 md:hidden" disabled={signingOut} onClick={signOut}><LogOut size={19} aria-hidden="true"/>{signingOut ? t('loggingOut') : t('logout')}</button>
        </div>
      </aside>
    ...
    <nav className="app-bottom-nav safe-bottom fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t px-1 pt-1.5 text-[10px] md:hidden">{mobilePrimaryLinks.map(([to, label, Icon]) => <NavLink key={to} to={to} end={to === '/'} className={({ isActive }) => `app-bottom-nav-link mx-0.5 flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-1.5 text-center font-semibold leading-tight ${isActive ? 'app-bottom-nav-link-active' : ''}`}><Icon size={20} aria-hidden="true" /><span className="max-w-full break-words text-center">{t(label)}</span></NavLink>)}<button type="button" className={`app-bottom-nav-link mx-0.5 flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-1.5 text-center font-semibold leading-tight ${open ? 'app-bottom-nav-link-active' : ''}`} aria-label={en ? 'More' : 'Thêm'} aria-pressed={open} onClick={() => setOpen(true)}><MoreHorizontal size={20} aria-hidden="true" /><span className="max-w-full break-words text-center">{t('more')}</span></button></nav>
  </div>;
}
