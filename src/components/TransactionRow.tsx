...
export function TransactionRow({
  ...
  return (
    <div>
      ...
      <div className={`transaction-table-row hidden w-full gap-1 border-t border-black/5 p-3 transition-colors hover:brightness-[.98] dark:border-white/5 dark:hover:brightness-110 md:grid md:min-w-[940px] md:items-center ${selectMode ? 'md:grid-cols-[32px_80px_minmax(160px,1fr)_130px_120px_145px_210px]' : 'md:grid-cols-[80px_minmax(160px,1fr)_130px_120px_145px_210px]'} ${tone.rowClass}`}>
        ...
        <div className="flex min-w-0 items-center gap-2">
          <strong className={`transaction-row-amount shrink-0 text-sm font-bold ${tone.amountClass}`}>{formatVnd(transaction.amount)}</strong>
          <span className="transaction-row-actions flex shrink-0 items-center gap-0">{showTrash && <button type="button" aria-label={`Khôi phục ${transaction.description}`} title="Khôi phục" className="p-2 text-emerald-700" onClick={() => { setOnlySelected(); onRestore(); }}><RotateCcw size={17}/></button>}{showTrash && <button type="button" aria-label={`Xóa vĩnh viễn ${transaction.description}`} title="Xóa vĩnh viễn" className="p-2 text-red-600" onClick={() => { setOnlySelected(); onPermanentlyDelete(); }}><Trash2 size={17}/></button>}<span className={showTrash ? 'hidden' : 'contents'}><button type="button" aria-label="Sao chép" className="p-2" disabled={copying} onClick={() => onCopy(transaction)}><Copy size={17}/></button>{canDelete && <button type="button" aria-label="Xóa" className="p-2 text-red-600" disabled={deleting} onClick={() => onRemove(transaction.id)}><Trash2 size={17}/></button>}</span></span>
        </div>
      </div>
    </div>
  );
...
function CatalogValue({ name, icon }: { name: string; icon?: string }) {
  const Icon = getCatalogIcon(icon);
  return <span className="flex min-w-0 items-center gap-1.5 truncate text-sm" title={name}><Icon className="catalog-value-icon shrink-0" size={15} aria-hidden="true" /><span className="min-w-0 truncate">{name}</span></span>;
}
