...
function CatalogTag({ name, icon }: { name: string; icon?: string }) {
  ...
  return <span className="transaction-card-tag max-w-full gap-1.5" title={name}><Icon className="catalog-tag-icon shrink-0" size={13} aria-hidden="true" /><span className="min-w-0 truncate">{name}</span></span>;
}
