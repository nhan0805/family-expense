...
export function ThemeSelect({ compact = false }: { compact?: boolean }) {
  ...
  const Switch = ({ checked, onClick, ariaLabel, kind }: { checked: boolean; onClick: () => void; ariaLabel: string; kind: 'theme' | 'language' }) => <button type="button" role="switch" aria-checked={checked} aria-label={ariaLabel} onClick={onClick} className={`relative inline-flex h-8 w-16 shrink-0 items-center rounded-full p-1 transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 ${kind === 'language' ? (checked ? 'bg-[#197254] dark:bg-[#8be9fd]' : 'bg-[#566579]') : (checked ? 'bg-[#197254] dark:bg-[#bd93f9]' : 'bg-[#c7cdd6] dark:bg-[#6272a4]')}`}><span className={`grid size-6 place-items-center rounded-full bg-white text-[10px] font-extrabold shadow-sm transition-transform ${checked ? 'translate-x-7' : 'translate-x-0'} ${kind === 'language' ? 'text-[#155e46] dark:text-[#282a36]' : ''}`}>{kind === 'theme' ? (checked ? <Moon size={14} className="text-[#155e46] dark:text-[#282a36]" aria-hidden="true" /> : <Sun size={14} className="text-amber-600" aria-hidden="true" />) : (checked ? 'EN' : 'VI')}</span></button>;
